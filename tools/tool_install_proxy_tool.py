...
class Tool(BaseTool):
    ...
    def install_proxy_tool(self):
        ...
        PrintUtils.print_delay("==========使用方法===========",0.001)
        PrintUtils.print_delay("1.桌面快捷方式：需要先右击允许执行才能使用",0.001)
        PrintUtils.print_delay("2.可以在终端中直接运行脚本启动,直接输入:$HOME/.clash/start_clash.sh",0.001)
        PrintUtils.print_delay("3.终端通过环境变量设置: export http_proxy=http://127.0.0.1:7890 && export https_proxy=http://127.0.0.1:7890",0.001)
        PrintUtils.print_delay("4.配置系统默认代理方式: 系统设置->网络->网络代理->手动->HTTP(127.0.0.1 7890)->HTTPS(127.0.0.1 7890)",0.001)
        PrintUtils.print_delay("5.其他系统（Android/IOS/Windows/MacOS）下载软件:https://repo.trojan-cdn.com/",0.001)
